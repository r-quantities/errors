...
test_that("covariances are correctly stored, retrieved and removed", {
  ...
  expect_true(attr(x, "id") == attr(z, "id"))
  expect_true(attr(x, "id") %in% ls(errors:::ht))
  expect_true(attr(y, "id") %in% ls(errors:::ht))
  expect_equal(length(ls(errors:::ht[[attr(x, "id")]])), 1)
  expect_true(attr(x, "id") %in% ls(errors:::ht[[attr(x, "id")]]))
  expect_equal(length(ls(errors:::ht[[attr(y, "id")]])), 1)
  expect_true(attr(y, "id") %in% ls(errors:::ht[[attr(y, "id")]]))
  ...
  gc()
  expect_equal(length(ls(errors:::ht)), 2)
  expect_equal(length(ls(errors:::ht[[attr(x, "id")]])), 2)
  expect_true(attr(y, "id") %in% ls(errors:::ht[[attr(x, "id")]]))
  expect_equal(length(ls(errors:::ht[[attr(y, "id")]])), 2)
  expect_true(attr(x, "id") %in% ls(errors:::ht[[attr(y, "id")]]))
